...
from app.api import admin_intents, admin_qa, chat, conversations, health
...
app.include_router(admin_intents.router, prefix=API)
...
@app.get("/chatbot_widget.js", include_in_schema=False)
def chatbot_widget_js():
    """Allintos CHATBOT_WIDGET_URL uyumu (Sinem base.html)."""
    path = _ROOT / "static" / "chatbot_widget.js"
    if not path.is_file():
        raise StarletteHTTPException(status_code=404, detail="chatbot_widget.js not found")
    return FileResponse(path, media_type="application/javascript")


@app.get("/chatbot_widget.css", include_in_schema=False)
def chatbot_widget_css():
    path = _ROOT / "static" / "chatbot_widget.css"
    if not path.is_file():
        raise StarletteHTTPException(status_code=404, detail="chatbot_widget.css not found")
