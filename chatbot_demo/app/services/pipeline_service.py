...
class V2IntentPipeline:
    ...
    def _fetch_kurumsal_hits_hybrid(self, ml_query: str, *, k: int = 50) -> list[VectorCandidate]:
        """Kurumsal K2 fallback — Allintos knowledge embeddings; NPZ yedek."""
        from app.core.config import allintos_local_fallback_enabled, get_k0_data_source

        if get_k0_data_source() == "allintos":
            try:
                from app.db.allintos_knowledge_store import search_corporate_knowledge

                qvec = self._embed_query(ml_query)
                hits = search_corporate_knowledge(qvec, top_k=k)
                if hits:
                    return hits
            except Exception as exc:
                print(f"[K2 Kurumsal] Allintos knowledge arama basarisiz: {exc}", flush=True)
