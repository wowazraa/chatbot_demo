"""
Optimized Benchmark Evaluation Script - Dynamic Two-Pass Cascade Router
Implements enhanced regex matching, dynamic scoring, and lowered base threshold.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. ENHANCED REGEX CONFIGURATION
# ─────────────────────────────────────────────────────────────────────────────
SECTOR_REGEX = {
    "health": re.compile(
        r"(hast.*|hsb.*|hbys.*|klinik.*|doktor.*|hekim.*|medikal.*|hasta.*|tıb.*|tıp.*|poliklinik.*|stok-takip.*|e-nabız.*)",
        re.IGNORECASE
    ),
    "defense": re.compile(
        r"(iha.*|siha.*|asker.*|savunma.*|radar.*|mühimmat.*|taktik.*|havelsan.*|aselsan.*|roketsan.*|stm.*|baykar.*|kargu.*)",
        re.IGNORECASE
    ),
    "education": re.compile(
        r"(obs.*|lms.*|öğrenci.*|okul.*|üniversite.*|karn.*|ders.*|akademik.*|eğitim.*|rektör.*|fakülte.*|e-okul.*|meb.*)",
        re.IGNORECASE
    ),
    "tourism": re.compile(
        r"(pms.*|otel.*|rezervasyon.*|acente.*|turizm.*|konaklama.*|tur.*|tatil.*|booking.*|otomasyon.*|oda-takip.*)",
        re.IGNORECASE
    )
}

def get_regex_match(query: str, predicted_sector: str) -> bool:
    """Check if query matches sector regex pattern."""
    pattern = SECTOR_REGEX.get(predicted_sector)
    if not pattern:
        return False
    return bool(pattern.search(query.lower()))

# ─────────────────────────────────────────────────────────────────────────────
# 2. DYNAMIC CASCADE ROUTER PIPELINE
# ─────────────────────────────────────────────────────────────────────────────
def evaluate_query(query: str, raw_bge_score: float, predicted_sector: str, 
                   base_threshold: float = 0.58, 
                   high_confidence_threshold: float = 0.80) -> tuple[bool, float]:
    """
    Sorguyu Dinamik Ağırlıklandırma Pipelinedan Geçirir.
    Returns: (is_accepted: bool, final_score: float)
    """
    final_score = raw_bge_score
    words = query.strip().split()
    
    # PASS 1: Yüksek Güvenlikli Otomatik Kabul
    if raw_bge_score >= high_confidence_threshold:
        return True, final_score

    # PASS 2: Gri Bölge (Dynamic Weighting & Regex Bonus/Penalty)
    has_regex_match = get_regex_match(query, predicted_sector)
    
    if has_regex_match:
        final_score += 0.18  # Strong Bonus for Regex Match
    else:
        final_score -= 0.12  # Penalty for non-match in Gri Zone

    # PASS 3: Short-Search Compensation (1-2 kelimelik gürültüsüz aramalar için)
    if len(words) <= 2 and has_regex_match:
        final_score += 0.05
...
def main():
    ...
    print("=== DYNAMIC TWO-PASS CASCADE ROUTER TESTİ BAŞLATILIYOR ===")
    ...
    print(f"Konfigurasyon: Reranker=OFF, Base Threshold=0.58, High Confidence=0.80", flush=True)
    print(f"Dynamic Bonus: +0.18 (regex match), Penalty: -0.12 (no match), Short: +0.05", flush=True)
    ...
    for i, item in enumerate(dataset):
        ...
        is_accepted, final_score = evaluate_query(query, raw_score, pred_sector, 
                                                 base_threshold=0.58, 
    ...
    print(f"Hatalı Kabul (FAR - Sızıntı): {stats['FAR']} / {n_ood} (%{far_pct:.2f})  <-- (Hedef: %0)")
    print(f"Hatalı Red (FRR - İş Kaybı): {stats['FRR']} / {n_b2b} (%{frr_pct:.2f})  <-- (Eski: %22.9)")
    ...
    # Sonuçları kaydet
    output_file = "benchmark_results_optimized.json"
    ...
    print("✓ Dynamic Two-Pass Cascade Router testi tamamlandı!")
