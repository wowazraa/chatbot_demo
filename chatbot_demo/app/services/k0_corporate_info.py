...
from app.core.config import get_allintos_site_url, get_k0_data_source
...
@lru_cache(maxsize=2)
def _load_records_cached(source: str) -> tuple[dict[str, Any], ...]:
    if source == "allintos":
        try:
            ...
            print(f"[K0] Allintos knowledge yuklenemedi, yerel fallback: {exc}", flush=True)

    if PREPARED_PATH.exists():
        data = json.loads(PREPARED_PATH.read_text(encoding="utf-8"))
        local = list(data.get("kayitlar") or [])
        if local:
            return tuple(local)
...
def invalidate_k0_records_cache() -> None:
    _load_records_cached.cache_clear()


def _load_records() -> list[dict[str, Any]]:
    source = get_k0_data_source()
    return list(_load_records_cached(source))
...
def _match_corporate_term(query: str, reply_lang: str = "tr") -> dict[str, Any] | None:
    ...
    return {
        ...
        "k0_source": rec.get("k0_source") or get_k0_data_source(),
    }
