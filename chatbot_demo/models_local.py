"""Yerel fallback ORM  pgvector yokken (embedding = Float[])."""
...
class Sector(Base):
    ...
    __table_args__ = (UniqueConstraint("sector_key", name="uq_sectors_sector_key"),)
...
class Blog(Base):
    ...
    author_id: Mapped[int | None] = mapped_column(ForeignKey("admin_users.id", ondelete="SET NULL"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class Conversation(Base):
    ...
    __table_args__ = (Index("idx_conversations_session_id", "session_id"),)
...
class AnalyticsEvent(Base):
    ...
    __table_args__ = (Index("idx_analytics_events_session_id", "session_id"),)
